...
#ifndef LLFRAME_BLAS_BLAS_EXTENSION_GPU_DIVIDE_VV_HPP
#define LLFRAME_BLAS_BLAS_EXTENSION_GPU_DIVIDE_VV_HPP
#include "core/base_type.hpp"
#include "cuda_runtime.h"
#include "core/exception.hpp"
#include "blas/blas_extension_gpu/cuda/include/core.cuh"
...
static constexpr void divide_vv(const int n, X *x, const int incx, Y *y,
    ...
    if (incx == 1 && incy == 1) {
        if constexpr (is_Same_Ty<int8_t, X, Y>) {
            cuda::divide_vv_i8i8(n, x, y, prop);
            return;
        }
        if constexpr (is_Same_Ty<int32_t, X, Y>) {
            cuda::divide_vv_i32i32(n, x, y, prop);
            return;
        }
        if constexpr (is_Same_Ty<float, X, Y>) {
            cuda::divide_vv_f32f32(n, x, y, prop);
            return;
        }
        if constexpr (is_Same_Ty<double, X, Y>) {
            cuda::divide_vv_f64f64(n, x, y, prop);
            return;
        }
...
#endif // LLFRAME_BLAS_BLAS_EXTENSION_GPU_DIVIDE_VV_HPP