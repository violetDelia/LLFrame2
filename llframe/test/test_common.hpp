...
using Exception_Tuple =
    std::tuple<llframe::exception::Bad_Alloc, llframe::exception::Bad_Parameter,
               ...
               llframe::exception::Null_Pointer, llframe::exception::Unhandled,
               llframe::exception::Unimplement, llframe::exception::Unknown,
               llframe::exception::Bad_Range, llframe::exception::CUDA_Error,
               llframe::exception::CuBLAS_Errot, llframe::exception::CuDNN_Errot>;

using Device_Tuple = std::tuple<llframe::device::CPU, llframe::device::GPU>;
using Type_Tuple = std::tuple<int, uint16_t, int8_t, uint64_t, float, double, std::string>;
using Arithmetic_Tuple = std::tuple<llframe::base_type::float32_t, llframe::base_type::float64_t,
...
#define APPLY_TUPLE_3(Tuple, Tp, Tp2, func, ...)                                                   \
    {                                                                                              \
        ...
        std::apply(                                                                                \
            [](auto &&...args) {                                                                   \
                (func<Tp, Tp2, std::remove_cvref_t<decltype(args)>>(##__VA_ARGS__), ...);          \
            },                                                                                     \
            tuple);                                                                                \
    };
#define APPLY_TUPLE_4(Tuple, Tp, Tp2, Tp3, func, ...)                                              \
    {                                                                                              \
        ...
        std::apply(                                                                                \
            [](auto &&...args) {                                                                   \
                (func<Tp, Tp2, Tp3, std::remove_cvref_t<decltype(args)>>(##__VA_ARGS__), ...);     \
            },                                                                                     \
            tuple);                                                                                \
    };
// 确保可以使用cuda分配内存
...
// 快速输出异常
#define PRINT_EXCEPTION(condition)                                                                 \
    try {                                                                                          \
        condition;                                                                                 \
...
void test_allocator_traits() {
    ASSERT_EQ((std::is_same_v<typename std::allocator_traits<Allocator>::const_pointer,
                              typename Allocator::const_pointer>),
              true);
    ASSERT_EQ((std::is_same_v<typename std::allocator_traits<Allocator>::const_void_pointer,
                              const void *>),
              true);
    ASSERT_EQ((std::is_same_v<typename std::allocator_traits<Allocator>::difference_type,
                              typename Allocator::difference_type>),
              true);
    ASSERT_EQ((std::is_same_v<typename std::allocator_traits<Allocator>::pointer,
    ...
    ASSERT_EQ((std::is_same_v<
              ...
              true);
    ASSERT_EQ(
        (std::is_same_v<typename std::allocator_traits<Allocator>::propagate_on_container_swap,
                        std::false_type>),
        true);
    ASSERT_EQ((std::is_same_v<typename std::allocator_traits<Allocator>::size_type,
                              typename Allocator::size_type>),
              true);
    ASSERT_EQ((std::is_same_v<typename std::allocator_traits<Allocator>::value_type,
                              typename Allocator::value_type>),
              true);
    ASSERT_EQ((std::is_same_v<typename std::allocator_traits<Allocator>::void_pointer, void *>),
              true);
};
