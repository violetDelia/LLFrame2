...
void test_Memory_ref(size_t device_id, Ty val, Ty val2) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        for (int i = 0; i < memory_ref.size(); i++) { memory_ref.set(i, val2); }
...
template <llframe::device::is_Device Device, class Ty>
void test_Memory_ref_each_type() {
    for (int i = 0; i < 10; i++) {
        if constexpr (llframe::is_Arithmetic<Ty>) {
            test_Memory_ref<Device, Ty>(i, 127, 126);
        }
        if constexpr (std::is_same_v<Ty, std::string>) {
            test_Memory_ref<Device, Ty>(i, "test", " ");
        }
    }
}

template <llframe::device::is_Device Device>
void test_Memory_ref_each_device() {
    APPLY_TUPLE_2(Type_Tuple, Device, test_Memory_ref_each_type);
}

TEST(Memory, ref) {
    APPLY_TUPLE(Device_Tuple, test_Memory_ref_each_device);
}
