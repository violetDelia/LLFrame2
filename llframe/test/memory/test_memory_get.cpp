...
void test_Memory_get(size_t device_id, Ty val) {
...
template <llframe::device::is_Device Device, class Ty>
void test_Memory_get_each_type() {
    for (int i = 0; i < 10; i++) {
        if constexpr (llframe::is_Arithmetic<Ty>) {
            test_Memory_get<Device, Ty>(i, 127);
        }
        if constexpr (std::is_same_v<Ty, std::string>) {
            test_Memory_get<Device, Ty>(i, "test");
        }
    }
}

template <llframe::device::is_Device Device>
void test_Memory_get_each_device() {
    APPLY_TUPLE_2(Type_Tuple, Device, test_Memory_get_each_type);
}

TEST(Memory, get) {
    APPLY_TUPLE(Device_Tuple, test_Memory_get_each_device);
}
