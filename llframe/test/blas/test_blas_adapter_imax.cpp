...
void test_blas_adapter_imax_for_each_device() {
    ...
    for (int i = 1; i < 24; i++) {
        ...
        if constexpr (!llframe::blas::is_Support_Openblas<Device, Ty>) {
            ASSERT_THROW(Blas_adapter::imax(n, x, 1, device),
                         llframe::exception::Unimplement);
            delete[] x;
            return;
        }
        Ty *null = nullptr;
        ASSERT_THROW(Blas_adapter::imax(n, null, 1, device),
                     llframe::exception::Null_Pointer);
        ASSERT_THROW(Blas_adapter::imax(n, x, -1, device),
                     llframe::exception::Bad_Parameter);
        IS_SAME(Device, llframe::device::CPU) {
            ASSERT_EQ(Blas_adapter::imax(n, x, 1, device), n - 1);
            ASSERT_EQ(Blas_adapter::imax(n / 2, x, 2, device), 1);
        }
        delete[] x;
    }
};
