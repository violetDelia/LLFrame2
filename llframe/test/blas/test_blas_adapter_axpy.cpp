...
void test_blas_adapter_axpy_for_each_device() {
    ...
    for (int i = 1; i < 24; i++) {
        ...
        if constexpr (!llframe::blas::is_Support_Openblas<Device, Ty>
            ...
            ASSERT_THROW(Blas_adapter::axpy(n, 1, x, 1, y, 1, device),
        ...
        Ty *null = nullptr;
        ASSERT_THROW(Blas_adapter::axpy(n, 2, null, 1, y, 1, device),
                     llframe::exception::Null_Pointer);
        ASSERT_THROW(Blas_adapter::axpy(n, 2, x, 1, y, -1, device),
                     llframe::exception::Bad_Parameter);
        IS_SAME(Device, llframe::device::CPU) {
            Blas_adapter::axpy(n, 2, x, 1, y, 1, device);
            for (int i = 0; i < n; i++) {
        ...
        IS_SAME(Device, llframe::device::GPU) {
            ...
            Blas_adapter::axpy(n, 2, gpu_x, 1, gpu_y, 1, device);
